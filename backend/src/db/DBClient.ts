		...
		return {
			...
			update: async (data: Partial<T>, where: Where<T>) => {
				if (!where || Object.keys(where).length === 0) {
					throw new Error(
						'update needs at least one condition in `where` field'
					);
				}
				...
				// Agora o erro se propaga corretamente
			...
			findUnique: async (where: Where<T>): Promise<T | null> => {
				...
				if (!where || Object.keys(where).length === 0) {
					throw new Error(
						'findUnique needs at least one condition in `where` field'
					);
				}
			...
			deleteMany: async (where?: Where<T>): Promise<boolean> => {
				// Monta o SQL base.
