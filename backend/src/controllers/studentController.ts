...
import {
  insertStudentIntoAClass,
  listStudentsInAClass,
...
export async function insertStudent(req: Request, res: Response) {
  try {
    ...
    const student = (await insertStudentIntoAClass(
...
export async function removeStudent(req: Request, res: Response) {
  try {
    ...
    if (!registration_id)
      throw new AppError(400, "registration_id is required.");

    // Chama o service responsável pela remoção do aluno da classe
    const removedStudent = await removeStudentFromAClass(
      registration_id,
      classId
    );
...
export async function updateStudentController(req: Request, res: Response) {
  try {
    // Busca o id pela rota
    const id = req.params.id;

    // Se não passar o id, rota está sendo chamada incorretamente
    if (!id) throw new AppError(400, "id is required in params.");
