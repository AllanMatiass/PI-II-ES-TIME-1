...
export async function findInstitutionByProfessorId(
	req: Request,
	res: Response
) {
	try {
...
export async function relateProfessorWithInstitution(
	req: Request,
	res: Response
) {
	try {
		...
		const institution: InstitutionResponseDTO =
			await insertProfessorInInstitution(professorId, institutionId);

		// Se tudo ocorrer bem, retorna aquela instituição recém vinculada
		return res.json({
			message:
				'Relationship between professor and institution was created successfully.',
			data: institution,
		});
	} catch (err: any) {
		if (err instanceof AppError) {
