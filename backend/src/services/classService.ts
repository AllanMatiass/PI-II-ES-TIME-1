...
const componentsValuesTable = db.table<GradeComponentValueDataModel>(
...
export async function ImportClass(data: StudentRegisterDTO[], id: string) {
  // Verifica se a turma existe uma única vez
  const classExist = await classTable.findUnique({ id });
  ...
  for (const row of data) {
    ...
    if (!student.name || !student.registration_id) continue;

    // Procura aluno existente pela matrícula/identificador
    let studentRecord = await studentsTable.findUnique({
      registration_id: student.registration_id,
    });

    // Se o aluno não existir, insere e recupera o registro criado
    if (!studentRecord) {
      const newId = await studentsTable.insert(student);
      // recupera o registro inserido para obter o id
      studentRecord = await studentsTable.findUnique({ id: newId });
    }

    // Se por algum motivo não conseguimos o registro, pula
    if (!studentRecord) continue;

    // Verifica se o aluno já está associado à turma
 ...
 */
export async function GetClassGradesForExport(
  classId: string,
  subjectId: string
): Promise<CSVResponseDTO[]> {
  ...
  for (const cs of classStudents) {
    ...
    const componentValues = await componentsValuesTable.findMany({
      student_id: student.id,
    });
